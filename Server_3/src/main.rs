...
async fn server_task(server1_id: usize) {
    let server_address: SocketAddr = format!("10.7.57.232:8080")
        .parse()
...
async fn start_election(server1_id: usize) -> bool {
    // Wrap the UdpSocket in Arc
    let socket1 = Arc::new(
        UdpSocket::bind("10.7.57.232:2113")
            .await
            .expect("Failed to bind server socket"),
    );
    ...
    // Wrap the UdpSocket in Arc
    let socket2 = Arc::new(
        UdpSocket::bind("10.7.57.232:2114")
            .await
            .expect("Failed to bind server socket"),
    );
    ...
    socket1
        .send_to(
            ...
            "10.7.57.6:2113", // Replace with actual addresses of other servers
        )
        .await
    ...
    socket2
        .send_to(
            &serde_json::to_string(&election_message).unwrap().as_bytes(),
            "10.7.57.249:2114", // Replace with actual addresses of other servers
        )
        .await
        .expect("Failed to send Election message");
