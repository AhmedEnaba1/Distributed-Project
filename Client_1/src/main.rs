...
async fn p2p(peer_socket: UdpSocket) {
    ...
    while let Ok((_bytes_received, client_address)) = peer_socket.recv_from(&mut buffer).await {
        ...
        while j < enc_total {
            println!("da5l");
            if let Ok((bytes_received, client_address)) = peer_socket.recv_from(&mut buffer).await
            {
                println!("AAAA");
                ...
                j += 1;
                peer_socket
                    .send_to("Ack".as_bytes(), client_address)
                    .await
...
async fn main() {
    ...
    let peer_socket = UdpSocket::bind("127.0.0.8:8085")
        .await
        .expect("Failed to bind middleware socket");
    ...
    while *termination.lock().unwrap() == 0 {
        ...
        if input.trim() == "2" {
            let client_socket_query = UdpSocket::bind("127.0.0.8:8091")
                .await
                .expect("Failed to bind client socket");
            query_online_users(client_socket_query, "Client1".to_string()).await;
            ...
            let connected_address = connect(client_socket_query, "Client1".to_string()).await;
            ...
            while j < enc_total {
                    ...
                    client_socket.recv_from(&mut request_buffer).await
                {
                    ...
                    let frag: frag = serde_json::from_str(&packet_string).unwrap();
